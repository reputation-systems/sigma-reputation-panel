...
export function generate_pk_proposition(wallet_pk: string): string {
  ...
  let hex = encodedProp.toHex();
  return "08cd" + hex.slice(2);
}
...
export async function check_if_r7_is_local_addr(
  value: string,
): Promise<boolean> {
  if (!get(connected)) return false;
  console.log(
    stringToRendered(
      generate_pk_proposition(await ergo.get_change_address()),
    ).substring(4),
  );
  console.log(stringToRendered(value));
  return (
    stringToRendered(
      generate_pk_proposition(await ergo.get_change_address()),
    ).substring(4) === stringToRendered(value)
  );
}
